...
def get_positional_inconsistency(opt, data):
	# Merge swaped subjects together
	...
	for keys, ex_pair in paired.items():
		...
		if key not in rs:
			rs[key] = []
		rs[key].append(abs(ex1_p00 - ex2_p01)) # p(x1,x2,a,t) = |S(x1|t1,2(a)) - S(x1|t2,1(a))|
		rs[key].append(abs(ex1_p01 - ex2_p00)) # p(x2,x1,a,t) = |S(x2|t1,2(a)) - S(x2|t2,1(a))|
		rs[key].append(abs(ex1_p10 - ex2_p11)) # p(x1,x2,na,t) = |S(x1|t1,2(na)) - S(x1|t2,1(na))|
		rs[key].append(abs(ex1_p11 - ex2_p10)) # p(x1,x2,na,t) = |S(x2|t1,2(na)) - S(x2|t2,1(na))|
...
def pairup_ex(data):
	...
	for keys, ex in data.items():
		keys = keys.lower().split('|')
		# tuple of subject genders
		scluster = (keys[0], keys[1])
		# tuple of subject names
		spair = (keys[2], keys[3])
		# ID of the context
		tid = keys[4]
		# a_cluster (always None in our case)
		acluster = keys[5]
		# attribute (property on which the bias is measured = occupation)
		opair = (keys[6], keys[7])

		# Check if the two subjects are different
		assert(spair[0] != spair[1])

		# Compute the new key with the names sorted, the ID, the cluster and the attributes
		key = (tuple(sorted([spair[0], spair[1]])), tid, acluster, opair[0], opair[1])
		if key not in paired:
			paired[key] = [None, None]

		# align examples to the order of spair key
		# paired[key] will not keep a "None" because we will always have swapped names:
		# ex: if there is Mary|James, there will be James|Mary in the file
		# Complete file needed
		if key[0][0] == spair[0]:
			paired[key][0] = ex
