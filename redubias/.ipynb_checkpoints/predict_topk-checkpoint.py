...
def predict(topk, batch_seq, batch_choices, tokeniser, model):
...
def predict_answers(source, topk, batch_size, tokeniser, model):
...
#   mask_filler = load_mask_filler(transformer_type)
  ...
  while cnt < num_ex:
    ...
    # with torch.no_grad():
    batch_output = predict(topk, batch_seq, batch_choices, tokeniser, model)
